...
var I18n;
(function (I18n) {
    I18n.APP_NAME = "LabelPlusFX Script";
    I18n.BUTTON_RUN = "导入";
    ...
    I18n.CHECKBOX_DIALOG_OVERLAY = "对指定分组涂白(多个分组以半角逗号隔开)：";
    I18n.COMPLETE = "导出完毕！";
    ...
    if (!(app.locale in { "zh_CN": 1, "zh_TW": 1, "zh_HK": 1 })) {
        ...
        I18n.CHECKBOX_RUN_ACTION = "Execute Automation Actions";
        ...
        I18n.CHECKBOX_DIALOG_OVERLAY = "Execute \"Dialog Overlay\" On Specified Groups (split groups with comma):";
...
(function (LabelPlus) {
    ...
    function meo2lp(path, meoFile) {
        var groups = [];
        for (var i = 0; i < meoFile.groupList.length; i++) {
            groups.push(meoFile.groupList[i].name);
        }
        var label_dict = {};
        for (var picName in meoFile.transMap) {
            var labels = [];
            for (var _i = 0, _a = meoFile.transMap[picName]; _i < _a.length; _i++) {
                var label = _a[_i];
                var l = {
                    x: label.x,
                    y: label.y,
    ...
    function parseMeoFile(path) {
        var f = new File(path);
        if (!f || !f.exists) {
            LabelPlus.log_err("LabelPlusFXJsonReader: file " + path + " not exists");
            return null;
        }
        f.open("r", "TEXT", "????");
        f.lineFeed = "unix";
        f.encoding = "UTF-8";
        var meo = f.read();
        var data = (new Function('return ' + meo))();
        f.close();
        return meo2lp(path, data);
    }
    function parseLPFile(path) {
        var f = new File(path);
        if (!f || !f.exists) {
            LabelPlus.log_err("LabelPlusFXTextReader: file " + path + " not exists");
            return null;
        }
        f.open("r");
        f.lineFeed = "unix";
        ...
        var parseTransLabel = function () {
            var s = lines[pointer].split(LABEL_END);
            var props = s[1].replace(PROP_START, "").replace(PROP_END, "").split(SPLIT);
            var index = parseInt(s[0].replace(LABEL_START, "").trim());
            var x = parseFloat(props[0].trim());
            var y = parseFloat(props[1].trim());
            var groupId = parseInt(props[2].trim()) - 1;
            pointer++;
            return {
                groupId: groupId,
                index: index,
        ...
        var parsePicHead = function () {
            var picName = lines[pointer].replace(PIC_START, "").replace(PIC_END, "");
            pointer++;
            return picName;
        };
        var parsePicBody = function () {
            var labels = [];
        ...
        var v = lines[pointer].split(SPLIT);
        var version = [parseInt(v[0].trim()), parseInt(v[1].trim())];
        pointer++;
        pointer++;
        var groupCount = 1;
        var groupList = [];
        ...
        pointer++;
        var comment = parseText([PIC_START]);
        var transMap = {};
        while (pointer < size && startWith(lines[pointer], PIC_START)) {
            var picName = parsePicHead();
    ...
    function parseTransFile(path) {
        if (path.substring(path.lastIndexOf("."), path.length) == '.json')
            return parseMeoFile(path);
        if (path.substring(path.lastIndexOf("."), path.length) == '.txt')
            return parseLPFile(path);
        return null;
    }
    LabelPlus.parseTransFile = parseTransFile;
})(LabelPlus || (LabelPlus = {}));
var LabelPlus;
(function (LabelPlus) {
    var opts = null;
    ...
    function importImage(img) {
        ...
        if (opts.dialogOverlayLabelGroups) {
            ...
            LabelPlus.log("do lp_dialogClear: " + points);
    ...
    function importFiles(custom_opts) {
        ...
        var lpFile = LabelPlus.parseTransFile(opts.lpTextFilePath);
        if (lpFile == null) {
        ...
        function LabelPlusInput() {
            ...
            _this.uiLpTextSelect = function (pnl) {
                ...
                pnl.lpTextFileBrowseButton.onClick = function () {
                    ...
                    var lpFile = LabelPlus.parseTransFile(f.fsName);
                    if (lpFile === null) {
                ...
                var getOption = function (opts, toFile) {
                    if (!toFile) {
                        ...
                        var lpFile = LabelPlus.parseTransFile(pnl.lpTextFileTextBox.text);
                        if (lpFile == null) {
                            alert(I18n.ERROR_PARSER_LPTEXT_FAIL);
                            return null;
                        }
            ...
            _this.uiAutomationPanel = function (pnl) {
                ...
                xx = xOfs;
                yy += 23;
                pnl.runActionGroupCheckBox = pnl.add('checkbox', [xx, yy, xx + 500, yy + 20], I18n.CHECKBOX_RUN_ACTION);
                ...
                yy += 23;
                {
                    ...
                    var func_1 = function () {
                        ...
                        pnl.overlayGroupAddGroupList.onChange = func_1;
                    };
                    pnl.overlayGroupAddGroupList.onChange = func_1;
                }
                var opts = _this.opts;
            ...
            _this.mainPannel = function (pnl) {
                ...
                _this.addToPickerList(ret.getOption);
                yy += 130;
                _this.stylePnl = pnl.add('panel', [xx, yy, xx + 480, yy + 180]);
                ret = _this.uiStylePanel(_this.stylePnl);
                _this.addToPickerList(ret.getOption);
                yy += 190;
                _this.automationPnl = pnl.add('panel', [xx, yy, xx + 480, yy + 160]);
